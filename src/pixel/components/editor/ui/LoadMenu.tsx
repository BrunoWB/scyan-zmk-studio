...
export const LoadMenu: React.FC<LoadMenuProps> = ({
  defaultOpen = false,
  onOpenLoadModal,
  onOpenImportModal,
  onExportPNG,
  ...
  return (
    <div className="relative" ref={menuRef}>
      {/* Modern Unified Load / Import / Export Button */}
      <button
        type="button"
        onClick={() => setIsOpen((prev) => !prev)}
        aria-haspopup="true"
        aria-expanded={isOpen}
        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs transition cursor-pointer shadow-xs border ${
          isOpen
            ? 'bg-[#1e2536] border-amber-400/70 text-white ring-1 ring-amber-400/30'
            : 'bg-[#141822] hover:bg-[#1c2230] border-[#242b3d] hover:border-[#354058] text-slate-200 hover:text-white active:scale-98'
        }`}
        title="Load, Import, or Export project assets"
        aria-label="Load project menu"
      >
        <FolderOpen className="w-3.5 h-3.5 text-amber-400" />
        <span className="font-medium tracking-wide">Load</span>
        <ChevronDown
      ...
      {isOpen && (
        <div
          ...
          className="absolute left-0 top-full mt-1.5 w-80 max-w-[calc(100vw-2rem)] bg-[#10131b]/95 backdrop-blur-md border border-[#242c3f] rounded-xl shadow-2xl shadow-black/80 p-1.5 z-50 flex flex-col gap-1 text-slate-200 select-none animate-in fade-in zoom-in-95 duration-100"
        >
          ...
          <div className="flex flex-col gap-0.5">
            ...
            <button
              type="button"
              role="menuitem"
              onClick={() => handleAction(onOpenLoadModal)}
              disabled={!onOpenLoadModal}
              className="flex items-center justify-between px-2 py-1.5 rounded-lg hover:bg-[#18202f] transition text-left cursor-pointer group disabled:opacity-30 disabled:cursor-not-allowed"
            >
              <div className="flex items-center gap-2.5 min-w-0">
                <div className="p-1 rounded bg-amber-500/15 border border-amber-500/30 text-amber-400 group-hover:scale-105 transition-transform shrink-0">
                ...
                <div className="flex flex-col min-w-0">
                  ...
                  <span className="text-[10px] text-slate-400 truncate">
                    Browse local rooms, peer counts & snapshots
                  </span>
                </div>
              </div>
              <span className="text-[10px] font-mono text-slate-500 bg-[#090b0e] px-1.5 py-0.5 rounded border border-[#1e2538] shrink-0 ml-2">
                Modal
              </span>
            </button>
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                <div className="p-1 rounded bg-cyan-500/15 border border-cyan-500/30 text-cyan-400 group-hover:scale-105 transition-transform shrink-0">
                  <Upload className="w-3.5 h-3.5" />
                </div>
                <div className="flex flex-col min-w-0">
                  <span className="text-xs font-medium text-slate-100 group-hover:text-cyan-300 transition-colors">
                    Import Image
                  </span>
                  <span className="text-[10px] text-slate-400 truncate">
                    PNG, JPG, BMP, GIF with auto-crop
                  </span>
                </div>
              </div>
              <span className="text-[10px] font-mono text-slate-500 bg-[#090b0e] px-1.5 py-0.5 rounded border border-[#1e2538] shrink-0 ml-2">
          ...
          <div className="flex flex-col gap-0.5">
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                <div className="p-1 rounded bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 group-hover:scale-105 transition-transform shrink-0">
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                <div className="p-1 rounded bg-emerald-500/10 border border-emerald-500/20 text-emerald-400/80 group-hover:scale-105 transition-transform shrink-0">
          ...
          {/* Section 3: Download Raster Images */}
          <div className="flex flex-col gap-0.5">
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                <div className="p-1 rounded bg-[#00f0ff]/15 border border-[#00f0ff]/30 text-[#00f0ff] group-hover:scale-105 transition-transform shrink-0">
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                <div className="p-1 rounded bg-slate-500/15 border border-slate-500/30 text-slate-300 group-hover:scale-105 transition-transform shrink-0">
                  <Image className="w-3.5 h-3.5" />
                </div>
                <div className="flex flex-col min-w-0">
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                <div className="p-1 rounded bg-purple-500/15 border border-purple-500/30 text-purple-400 group-hover:scale-105 transition-transform shrink-0">
                  <Image className="w-3.5 h-3.5" />
                </div>
                <div className="flex flex-col min-w-0">
          ...
          {/* Section 4: Export Firmware & Code */}
          <div className="flex flex-col gap-0.5">
            <div className="px-2 pt-1 text-[9px] font-semibold tracking-wider text-amber-400/90 uppercase">
              Export Firmware
            </div>
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                <div className="p-1 rounded bg-amber-500/10 border border-amber-500/20 text-amber-400/80 group-hover:scale-105 transition-transform shrink-0">
